/*
 * CSE 5441 : Lab 2
 * Filename : biswas_rajarshi_persistent.cc
 * Author   : Rajarshi Biswas (biswas.91@osu.edu)
 * 	      The Ohio State University.
 */

#include "biswas_rajarshi_persistent.h"
#include <pthread.h>
 ...
 */
int find_contact_distace(int current_box_id,
...
int do_stencil_computation() {
	...
	for (int thread_id = 0; thread_id < number_of_threads; thread_id++) {
		result  = pthread_create(&threads[thread_id], &thread_attr,
	...
	// Await Thread Exit.
	for (int thread_id  = 0; thread_id < number_of_threads; thread_id++) {
		result = pthread_join(threads[thread_id], NULL);
		assert(result == 0);
	}
...
int main(int argc, char* argv[]) {
	...
	/* Read affect_rate and epsiloni and number of threads. */
